...
function validationArrayProps(object, props) {
	...
	props.forEach(function (prop) {
		...
		if ((typeof prop === 'undefined' ? 'undefined' : _typeof(prop)) === 'object' && prop !== null) {
			var localResult = validationObjectProps(result.result, prop, propsName);
		...
		if ({}.hasOwnProperty.call(props, key)) {
			if (validationProp(object, key)) {
				...
				if (props[key] instanceof Array) {
					...
					var localResult = validationArrayProps(result.result[key], props[key], propsName + '.' + key);
				...
				if (_typeof(props[key]) === 'object' && props[key] !== null) {
					...
					var _localResult = validationObjectProps(result.result[key], props[key], propsName + '.' + key);
				...
				if (props[key] instanceof Array) {
					result.result[key] = validationArrayProps(result.result[key], props[key], propsName + '.' + key).result;
					return true;
				}

				if (_typeof(props[key]) === 'object' && props[key] !== null) {
					var _localResult2 = validationObjectProps(result.result[key], props[key], propsName + '.' + key);
